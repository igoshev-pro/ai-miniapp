...
const modelCapabilities: Record<string, {
  ...
  'suno-v4': {
    type: 'music',
    maxDuration: 240, minDuration: 30,
    ...
    voices: [],
    languages: ['ru', 'en', 'es', 'fr', 'de', 'ja', 'ko', 'zh'],
    supportsLyrics: true,
    supportsInstrumental: true,
  },
  'elevenlabs': {
    type: 'tts',
    maxDuration: 300, minDuration: 5,
    genres: [],
    moods: [],
...
export function AudioGenerationPage() {
  ...
  useEffect(() => {
    setGenre('')
    setMood('')
    setDuration(isMusic ? 60 : 30)
    setIsInstrumental(false)
    setLanguage('ru')
    setVoice(caps.voices[0] || 'Rachel')
    setStability(50)
    setSimilarity(75)
  ...
  // Стоимость зависит от длительности для TTS
        ...
        {audioGenerations.length === 0 && (
          <div className="gen-page__empty fade-in fade-in--2">
            ...
            <div className="gen-page__empty-title">
              {isMusic ? 'Генерация музыки' : 'Озвучка текста'}
            </div>
                ...
                <>
                  ...
                  <div className="gen-field">
                    ...
                    <div className="gen-field__chips gen-field__chips--wrap">
                      <button
                        className={`gen-chip ${!genre ? 'gen-chip--active' : ''}`}
                        onClick={() => { setGenre(''); haptic('light') }}
                      >
                        Авто
                      </button>
                      {caps.genres.map((g) => (
                        <button
                          key={g}
                          className={`gen-chip ${genre === g ? 'gen-chip--active' : ''}`}
                          onClick={() => { setGenre(g); haptic('light') }}
                        >
                          {genreLabels[g] || g}
                        </button>
                      ))}
                  ...
                  <div className="gen-field">
                    ...
                    <div className="gen-field__chips gen-field__chips--wrap">
                      <button
                        className={`gen-chip ${!mood ? 'gen-chip--active' : ''}`}
                        onClick={() => { setMood(''); haptic('light') }}
                      >
                        Авто
                      </button>
                      {caps.moods.map((m) => (
                        <button
                          key={m}
                          className={`gen-chip ${mood === m ? 'gen-chip--active' : ''}`}
                          onClick={() => { setMood(m); haptic('light') }}
                        >
                          {moodLabels[m] || m}
                        </button>
                      ))}
                  ...
                  <div className="gen-field">
                    ...
                    <div className="gen-field__range-labels">
                      <span>{caps.minDuration}с</span>
                      <span>{caps.maxDuration}с (4 мин)</span>
                    </div>
                  </div>
                  ...
                  {caps.supportsInstrumental && (
                    <div className="gen-field">
                      ...
                      <div className="gen-field__chips">
                        <button
                          className={`gen-chip ${!isInstrumental ? 'gen-chip--active' : ''}`}
                          onClick={() => { setIsInstrumental(false); haptic('light') }}
                        >
                          🎤 С вокалом
                        </button>
                        <button
                          className={`gen-chip ${isInstrumental ? 'gen-chip--active' : ''}`}
                          onClick={() => { setIsInstrumental(true); haptic('light') }}
                        >
                          🎵 Инструментал
                        </button>
                      </div>
                    ...
                    <div className="gen-field">
                      ...
                      <div className="gen-field__chips gen-field__chips--wrap">
                        {caps.languages.map((l) => (
                          <button
                            key={l}
                            className={`gen-chip ${language === l ? 'gen-chip--active' : ''}`}
                            onClick={() => { setLanguage(l); haptic('light') }}
                          >
                            {langLabels[l] || l}
                          </button>
                        ))}
                  ...
                  <div className="gen-field">
                    ...
                    <div className="gen-field__chips gen-field__chips--wrap">
                      {caps.voices.map((v) => (
                        <button
                          key={v}
                          className={`gen-chip ${voice === v ? 'gen-chip--active' : ''}`}
                          onClick={() => { setVoice(v); haptic('light') }}
                        >
                          {v}
                        </button>
                      ))}
